...
const { Proxy, config, getLogger } = import("../src/index.js").then(
	(index_) => {
		const { Proxy, config, getLogger } = index_;
		...
		function printBanner() {
			...
			console.log(
				"                      " +
					chalk.hex(MAIN_COLOR).bold("Green") +
					" " +
					chalk.bold.white("Tunnel"),
			);
		}
		...
		function printAlert(proxy) {
			...
			console.log(
				"    " +
					chalk
						.bgHex(MAIN_COLOR)
						.black(" Note: GreenTunnel does not hide your IP address "),
			);
			console.log(
				"      " +
					chalk.hex(MAIN_COLOR)(
			...
			console.log(
				"\n      " +
					chalk.white(
						` GreenTunnel is running at ${proxy.server.address().address}:${proxy.server.address().port}. `,
					),
			);
		}
